...
last_updated: 2019-06-27 23:05:00 +0300
...
- [Support]({{ site.baseurl }}/qemu/support/)
- [How to build?](https://github.com/xpack-dev-tools/qemu-xpack/blob/xpack/README-BUILD.md)
- [How to publish?](https://github.com/xpack-dev-tools/qemu-xpack/blob/xpack/README-PUBLISH.md)
...
missing, so it could not be used with the GNU MCU Eclipse plug-ins.

## Overview

The **xPack QEMU** is a fork of the public open-source 
...
The main benefits of using the **xPack QEMU** are:

- users can run the **blinky** projects generated by the GNU MCU 
  ...
  etc. are supported.
- the emulator provides a very convenient platform for running 
  **unit tests**, fully supporting the **semihosting** features 
  required for this.

## Supported boards and MCUs

The boards currently supported by the xPack QEMU are:
...
The xPack QEMU maintains a certain degree of compatibility with the 
...
The **blinky** projects generated by the GNU MCU Eclipse templates 
(and all STM32 HAL projects) use the SysTick to measure time, so any 
realistic emulation should support SysTick. xPack QEMU not only does 
...
both located in the doc folder in the install location. The official 
QEMU site also has a page with various links to 
[QEMU manuals](http://wiki.qemu.org/Manual).

All provide a lot of information about QEMU, but do not go into much 
detail related to Cortex-M.

## Download

All **xPack QEMU** versions are available from the GitHub 
...
The details of installing the **xPack QEMU** on various platforms are 
presented in a dedicated [Install]({{ site.baseurl }}/qemu/install) page.

## Testing

The recommended method to test QEMU is using the new QEMU Debugging 
plug-in; create a test project using the 
[blinky](https://gnu-mcu-eclipse.github.io/tutorials/blinky-arm/) 
**STM32F4xx C/C++ Project** template or even the 
**Hello World ARM Cortex-M C/C++ Project** template and run it as 
explained in the 
[QEMU debugging Eclipse plug-in](https://gnu-mcu-eclipse.github.io/debug/qemu/) page.
...
[support]({{ site.baseurl }}/qemu/support/) page.

## Change log

The change log is available in the repository
[`CHANGELOG.md`](https://github.com/xpack-dev-tools/qemu-xpack/blob/xpack/CHANGELOG.md) file.
...
[scripts](https://github.com/xpack-dev-tools/qemu-xpack/tree/xpack/scripts/)
folder.
