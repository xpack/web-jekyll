...
Since the `last_update_at` plug-in is not available on GitHub Pages, the
`last_updated:` fields must be adjusted before pushing the code to GitHub.

```console
$ bash scripts/adjust-timestamps.sh
```

This script will enumerate all files in `pages` and `_posts` and adjust
the `last_updated:` field to the current date, if necessary. Then it'll
stage the files and commit them all at the end.
